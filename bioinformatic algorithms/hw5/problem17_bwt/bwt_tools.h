...
class BWTBuilder{
  public:
    ...
    void Build(std::string const& str) {
      ...
      _suffix_array = getSuffixArray(str);
...
//      std::cout << _fcolumn << std::endl  << _lcolumn << std::endl;
    ...
    std::basic_string<size_t> getSuffixArray(std::string const & str) {
      std::basic_string<size_t> suffices_positions;
      suffices_positions.resize(str.size());
      for (size_t i = 0; i < str.size(); i++) {
        suffices_positions[i] = i;
      }
      _sort(str, suffices_positions, 0, 0, str.size());
      return suffices_positions;
    }

    void _sort(std::string const & str, std::basic_string<size_t> & suffices_positions, size_t start_pos, size_t offset, size_t end_pos) {
      ...
      std::vector<size_t> temp(suffices_positions.size(), 0);
      for (size_t i = start_pos; i < end_pos; i ++) {
