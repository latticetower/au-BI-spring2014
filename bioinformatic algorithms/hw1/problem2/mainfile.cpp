...
std::string get_result_string(std::string const& str, std::vector<std::pair<ArrowDirection, int> > & path, int n) {
  std::string buffer = "";
  int i = 0;
  for (std::vector<std::pair<ArrowDirection, int> >::reverse_iterator iter = path.rbegin(); iter != path.rend(); ++iter) {
...
std::string get_result_string2(std::string const & str_a, std::string const & str_b, std::vector<std::pair<ArrowDirection, int> > & path, int n) {
...
int main(int argc, char** argv) {
  ...
  std::pair<int, std::vector<std::pair<ArrowDirection, int> > > result = estimator.levenshtein_dist(fasta.data[0].second, fasta.data[1].second);
  if (result.first >= 0)
