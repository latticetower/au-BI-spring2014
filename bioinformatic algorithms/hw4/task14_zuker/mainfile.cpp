...
int main(int argc, char** argv) {
    ...
    std::vector<StackedSeqPair> result = zuker_predictor.get_result(0, fasta.data[0].second.size() - 1, fasta.data[0].second);
    ...
    for (size_t i = 0; i < result.size(); i++) {
        output_file << result[i].first.first << "," << result[i].first.second << "-"
                    << result[i].second.first << "," << result[i].second.second << std::endl;
    }
